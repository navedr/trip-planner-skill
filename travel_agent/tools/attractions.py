"""Attraction search wrapper (DuckDuckGo -> TripAdvisor) — returns structured dicts."""

from __future__ import annotations

import time
import urllib.parse

from ._selenium import DEFAULT_GRID_URL, create_driver


def _build_search_url(destination: str, interest: str | None) -> str:
    """Build a DuckDuckGo query for TripAdvisor results.

    DuckDuckGo, not Google: Google blocks the Selenium Grid's datacenter exit IP
    with an "unusual traffic from your computer network" interstitial, so the
    Grid gets zero results. DuckDuckGo's HTML endpoint serves datacenter IPs fine.
    """
    if interest:
        query = f'site:tripadvisor.com {interest} "{destination}"'
    else:
        query = f'site:tripadvisor.com things to do "{destination}"'
    return f"https://duckduckgo.com/html/?q={urllib.parse.quote(query)}"


def search_attractions(
    destination: str,
    interest: str | None = None,
    grid_url: str = DEFAULT_GRID_URL,
) -> dict:
    """Search DuckDuckGo for TripAdvisor attraction results and return snippets.

    Returns:
        {"search_url": str, "results": [str], "count": int}
    """
    url = _build_search_url(destination, interest)

    driver = create_driver(grid_url)
    try:
        driver.get(url)
        time.sleep(6)

        results = driver.execute_script("""
            const snippets = [];
            const items = document.querySelectorAll('.result, .web-result, #links > div');
            items.forEach((el, i) => {
                if (i >= 15) return;
                const text = el.innerText.trim();
                if (text.length > 20) snippets.push(text.substring(0, 500));
            });
            return snippets;
        """)

        return {"search_url": url, "results": results, "count": len(results)}
    finally:
        driver.quit()


def search_attractions_firecrawl(
    destination: str,
    interest: str | None = None,
) -> dict:
    """Search Google via Firecrawl. Same return shape as search_attractions."""
    from ._firecrawl import scrape_url

    url = _build_google_url(destination, interest)
    markdown = scrape_url(url, wait_for_ms=8000)

    blocks = [b.strip() for b in markdown.split("\n\n") if len(b.strip()) > 30]
    results = [b[:600] for b in blocks[:15]]
    if not results:
        results = [markdown[:4000]]

    return {"search_url": url, "results": results, "count": len(results)}
